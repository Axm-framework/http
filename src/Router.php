...
class Router
{
    ...
    /**
     * Obtiene el callback de una route.
     *
     * @param string $method
     * @return array
     */
    ...
    /**
     * Busca si existe Middlewares en en controllers y los ejecuta.
     *
    ...
    private static function invokeRouteWithSingleCallback(string $method): mixed
    {
        ...
        // Llamar al método render() en la instancia del controlador
    ...
    private static function invokeRouteWithMethodCall(): mixed
    {
        ...
        // static::$app->controller = function () use ($controller) {
        //     return new $controller();
        // };

        if (!method_exists($controller, $method)) {
            throw new AxmException(
                Axm::t('axm', 'El método "%s" no existe en %s.', [
                    $method,  $controller
                ])
            );
        }

        // $controller->addAction($method);
    ...
    /**
     * Verifica los parámetros en las rutas, los fracciona y retorna el callback. 
     *
     * @return array|false
     */
    public static function getCallBack($method, $uri)
    {
